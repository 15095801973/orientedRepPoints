...
def prepare_multi_scale_data(src_path, dst_path, gap=200, subsize=1024, scales=[0.5, 1.0, 1.5], num_process=32):
    ...
    split_train = splitbase_trainval(osp.join(src_path, ''), dst_trainval_path,
                                     gap=gap, subsize=subsize, num_process=num_process)
    for scale in scales:
        split_train.splitdata(scale)
    ...
    DOTA2COCOTrain(dst_trainval_path,
                   osp.join(dst_trainval_path, 'trainval.json'),
                   wordname_15)
    # DOTA2COCOTest(dst_test_base_path,
    #               osp.join(dst_test_base_path, 'test.json'),
    #               wordname_15)
    # print('please generate labels with coco format')
...
if __name__ == '__main__':
    ...
    print('start')
    src_path = r'F:\360downloads\part1(val_first100'
    out_path = r'F:\360downloads\OrientedRepPoints-main\data\dota_1024_first100_train_val'
    prepare_multi_scale_data(src_path,
                             out_path, subsize=1024,
                             gap=200, scales=[1.0], num_process=8)
    print('done')
